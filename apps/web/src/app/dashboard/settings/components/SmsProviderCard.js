...
function SmsProviderCard({ 
  ...
  const handleToggleEnabled = async (checked) => {
    try {
      await smsConfigApi.updateProvider(provider.provider, { isEnabled: checked });
        ...
        {isEditing && (
          <div className='space-y-4 pt-4 border-t border-border'>
            ...
            <div className='flex items-center justify-between'>
              <Label htmlFor={`${provider.provider}-isDefault`}>设为默认短信服务</Label>
              <Switch
        ...
        {!isEditing && config.signName && (
          <div className='text-sm text-muted-foreground space-y-1 pt-4 border-t border-border'>
            ...
            {isAliyun && config.region && (
              <div className='text-xs'>地域: {config.region}</div>
            )}
            {isTencent && config.appId && (
              <div className='text-xs'>应用 ID: {config.appId}</div>
            )}
          </div>
        )}
      </div>
