...
import { deductCredits, getCreditConfig } from '../../extensions/credits/services/creditService.js';
...
export default async function postRoutes(fastify, options) {
  ...
  fastify.post('/', {
    ...
    if (postNumber > 1) {
      try {
        const replyCreditChange = await getCreditConfig('post_reply_amount', 2); // 默认值2，表示回复奖励2积分
        ...
        if (err.message.includes('余额不足')) {
           return reply.code(400).send({ error: `积分余额不足，发表回复需要 ${Math.abs(await getCreditConfig('post_reply_amount', 0))} 积分` });
        }
